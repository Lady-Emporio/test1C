...
int API_screenToFile(const wchar_t* filePath) {
	...
	if (hdc == NULL) {
		myLog("Couldn't create device context\n");
		return 1;
	}
	...
	if (hdc2 == NULL) {
		DeleteDC(hdc);
		myLog("Couldn't create compatible device context\n");
		return 1;
	}
	...
	if (bitmap == NULL) {
		DeleteDC(hdc);
		DeleteDC(hdc2);
		myLog("Couldn't create compatible bitmap\n");
		return 1;
	}
	...
	if ((gdiobj == NULL) || (gdiobj == (void*)(LONG_PTR)GDI_ERROR)) {
		DeleteDC(hdc);
		DeleteDC(hdc2);
		myLog("Couldn't select bitmap\n");
		return 1;
	}
	if (!BitBlt(hdc2, 0, 0, dwWidth, dwHeight, hdc, 0, 0, SRCCOPY)) {
		DeleteDC(hdc);
		DeleteDC(hdc2);
		myLog("Could not copy bitmap\n");
		return 1;
	}
	...
	myLog("Bitmap captured to disk file\n");
	return 0;
}
...
void API_BMP_to_PNG(const wchar_t* path_BMP, const wchar_t* path_PNG) {
	...
	if (stat == 0)
		myLog("Bird.png was saved successfully\n");
	else
		myLog("Failure: stat = "+std::to_string(stat));
...
void API_screenToPNG(const wchar_t* resultPathPng) {
	...
	if (0 == r) {
		myLog( "error with GetTempFileName.");
		return;
	}
